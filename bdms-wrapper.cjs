...
logger.info('worker receive:', workerData)
// xmst = workerData.xmst

function defineObjectToString(obj, key) {
...
defineObjectToString(document, 'HTMLDocument')
//localStorage
localStorage = {
    getItem: function (key) {
        return key === "xmst" ? workerData.xmst : null;
    }
};
this.funcSetNative(localStorage.getItem);
defineObjectToString(localStorage, 'LocalStorage')
//其他对象
sessionStorage = defineObjectToString({}, 'SessionStorage')
location = defineObjectToString({
    "ancestorOrigins": {},
...
navigator = defineObjectToString({ userAgent: workerData.userAgent }, 'Navigator')
history = defineObjectToString({}, 'History')
screen = defineObjectToString({
    availHeight: 1032,
    availLeft: 0,
    availTop: 0,
    availWidth: 1920,
...
//     const xmst = 'OADdm860jHJIl85Dloqxndsvq_qLVGkPLrSrqkhiBpTp4_oiiGQJTPxBwTdVWkZ28rI0TAl6A5kfu0i8_kh1H-jQcM7oQv25uYMEK-OR2Pg33GCl6pdwa88NnAb3zBd4s9yL378wd_x5N--T7mwyjkUT10T9rq_yAfSZrL9vXF5Cbw2aqxDKCL0=';
//     const userAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36'
//     const rawABogus = getABogus(baseUrl, xmst, userAgent)
...
//     logger.info("请求的完整URL：", targetUrl);
//     //5.定义请求配置（与之前一致，无需修改）
//     const options = {
...
//         logger.info("请求成功，状态码：", res.statusCode);
...
//         res.on('end', () => {
//             try {
//                 logger.info(`RawResponseBody: [${responseData}]`);
//                 //解析JSON格式响应
...
//         //监听响应结束事件，处理完整响应数据
//         res.on('end', () => {
//             logger.info('请求状态码:', res.statusCode);
//             logger.info('响应头:', res.headers);
//             try {
//                 //解析JSON格式响应（接口返回application/json）
//                 const result = JSON.parse(responseData);
//                 logger.info('响应数据（格式化）:', JSON.stringify(result, null, 2));
//             } catch (err) {
//                 //非JSON格式数据直接打印
//                 logger.info('响应数据（原始）:', responseData);
//             }
//         });
//     });
...
parentPort.on('message', url => {
    try {
        const aBogus = getABogus(url)
        logger.info('worker calc a_bogus', aBogus)
        parentPort.postMessage(aBogus)
    } catch (error) {
        logger.error(error)
    }
})
...
//     parentPort.postMessage(getFeedBogus(msg.url, msg.xmst, msg.userAgent))
// });
